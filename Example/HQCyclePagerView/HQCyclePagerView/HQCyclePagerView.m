...
// iOS 内联函数
/*
 内联函数的好处就是没有普通函数调用的额外开销，OC是一种消息类型的语言调用函数需要很多额外的操作
 宏定义是预处理器进行替代的，而内联函数是编译器控制实现的，只有在用到的时候才会展开插入，去掉了函数参数的压栈，减小了开销，我们可以像函数一样调用内联函数，而不用考虑宏一样的问题
 ps:a 如果函数体内的代码比较长，使用内联将导致内存消耗代价较高; b 如果函数体内出现循环，那么执行函数体内代码的时间要比函数调用的开销大;
 */

NS_INLINE BOOL HQEqualIndexSection(HQIndexSection indexSection1,HQIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE HQIndexSection HQMakeIndexSection(NSInteger index, NSInteger section) {
    HQIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface HQCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HQCyclePagerTransformLayoutDelegate> {
    // 优化使用struct来保存delegate是否response   BOOL = 1 个字节
    struct {
...
@property (nonatomic, assign) HQIndexSection beginDragIndexSection;
...
@implementation HQCyclePagerView
...
- (void)addCollectionView {
    HQCyclePagerTransformLayout *layout = [[HQCyclePagerTransformLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:HQPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (HQCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<HQCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((HQCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<HQCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(HQPagerScrollDirection)direction animate:(BOOL)animate {
    HQIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:HQMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:HQMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)updateLayout {
    ...
    ((HQCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(HQIndexSection)indexSection {
...
- (HQIndexSection)nearlyIndexPathAtDirection:(HQPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (HQIndexSection)nearlyIndexPathForIndexSection:(HQIndexSection)indexSection direction:(HQPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == HQPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? HQMakeIndexSection(0, 0) : indexSection;
        } else if (direction == HQPagerScrollDirectionRight) {
            return HQMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? HQMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return HQMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == HQPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return HQMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return HQMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return HQMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return HQMakeIndexSection(indexSection.index, 0);
    }
    return HQMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (HQIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return HQMakeIndexSection(0, 0);
    }
    ...
    return HQMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(HQIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:HQMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(pagerView:didSelectedItemCell:atIndexSection:)]) {
        [_delegate pagerView:self didSelectedItemCell:cell atIndexSection:HQMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    HQIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    HQIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !HQEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !HQEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    HQPagerScrollDirection direction = HQPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = HQPagerScrollDirectionLeft;
    }
    HQIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - HQCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((HQCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
