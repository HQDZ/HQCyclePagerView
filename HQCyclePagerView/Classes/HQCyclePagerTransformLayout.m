...
typedef NS_ENUM(NSUInteger, HQTransformLayoutItemDirection) {
    HQTransformLayoutItemLeft,
    HQTransformLayoutItemCenter,
    HQTransformLayoutItemRight,
};


@interface HQCyclePagerTransformLayout () {
...
@interface HQCyclePagerViewLayout ()

@property (nonatomic, weak) UIView *pageView;

@end

@implementation HQCyclePagerTransformLayout
...
- (void)setDelegate:(id<HQCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(HQCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (HQTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    HQTransformLayoutItemDirection direction= HQTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = HQTransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = HQTransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == HQCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != HQCyclePagerTransformLayoutNormal) {
        ...
        for (UICollectionViewLayoutAttributes *attributes in attributesArray) {
            ...
            if (_delegateFlags.applyTransformToAttributes) {
                ...
                [self applyTransformToAttributes:attributes layoutHQpe:_layout.layoutType];
            }
        }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != HQCyclePagerTransformLayoutNormal){
        [self initializeTransformAttributes:attributes layoutHQpe:_layout.layoutType];
    }
    return attributes;
}
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutHQpe:(HQCyclePagerTransformLayoutType)layoutHQpe {
    switch (layoutHQpe) {
        case HQCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case HQCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutHQpe:(HQCyclePagerTransformLayoutType)layoutHQpe {
    switch (layoutHQpe) {
        case HQCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        HQTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case HQTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case HQTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation HQCyclePagerViewLayout
